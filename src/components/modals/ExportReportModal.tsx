...
export const ExportReportModal: React.FC = () => {
  const { isExportModalOpen, setIsExportModalOpen, summary, forecast, riskPrediction, dataset, formatCurrency, currency, showToast } = useFinancial();
...
--------------------------------------------------
- Current Available Liquidity: ${formatCurrency(summary.currentBalance)}
- Configured Safe Cash Buffer: ${formatCurrency(summary.safeBufferThreshold)}
...
- Monthly Baseline Inflow: ${formatCurrency(summary.monthlyInflow)}
- Monthly Baseline Outflow: ${formatCurrency(summary.monthlyOutflow)}
...
- Lowest Projected Balance Point: ${formatCurrency(forecast.lowestProjectedPoint)}
...
Generated via CashFlow Guardian AI Engine`;
  ...
  const handleDownloadCSV = () => {
    const headers = [`Date`, `Day Index`, `Projected Balance (${currencyLabel})`, `Predicted Inflow (${currencyLabel})`, `Predicted Outflow (${currencyLabel})`, `Net Flow (${currencyLabel})`, `Risk Level`, `Below Safe Buffer`];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-fade-in">
      <div className="w-full max-w-xl rounded-2xl bg-white p-6 shadow-2xl border border-slate-200">
        ...
        {/* Quick Options */}
        <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3 font-sans">
          <button
          ...
          >
            ...
            <div>
              <div className="text-sm font-semibold text-slate-900">Download CSV Forecast</div>
              <div className="text-xs text-slate-500">{forecast.forecastDays.length}-day daily ledger data</div>
            </div>
          </button>
          ...
          >
            ...
            <div>
              <div className="text-sm font-semibold text-slate-900">{copiedSummary ? 'Copied brief!' : 'Copy executive brief'}</div>
              <div className="text-xs text-slate-500">Formatted for email / board review</div>
            </div>
          </button>
        </div>
        ...
        <div className="mt-4 font-sans">
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className="text-xs text-slate-400 font-mono">{dataset.name}</span>
          </div>
          <pre className="h-44 overflow-y-auto rounded-xl border border-slate-200 bg-slate-900 p-3 text-xs font-mono text-emerald-400 leading-relaxed custom-scrollbar">
            {executiveBrief}
          </pre>
        </div>

        <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-end font-sans">
          <button
            onClick={() => setIsExportModalOpen(false)}
